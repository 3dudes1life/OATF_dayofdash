...
function renderIssues() {
  ...
  const pillEl = $('#issueStatusPill');

  if (countEl) {
    countEl.textContent = `${count} Open ${count === 1 ? 'Issue' : 'Issues'}`;
  }

  if (textEl) {
    textEl.textContent = count
      ? `${count === 1 ? 'One issue needs' : 'Multiple issues need'} production attention.`
      : 'Production is clear.';
  }

  if (pillEl) {
    pillEl.textContent = count ? 'ACTION' : 'CLEAR';
    pillEl.className = `issue-status-pill ${count ? 'alert' : 'clear'}`;
  }
